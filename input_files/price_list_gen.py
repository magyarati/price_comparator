...
def process_weekly_data(store, selected_dataset, current_date, is_forward):
    """Generates weekly price lists and discounts based on the current date."""
    percent_change = random.randint(0, 3)
    if is_forward:
        price_adjustment = 1 + percent_change / 100
    else:
        price_adjustment = 1 - percent_change / 100

    selected_dataset["price"] = (selected_dataset["price"] * price_adjustment).round(2)
...
for store in stores:
    ...
    # Copy original price list files to the target directory
    shutil.copyfile(f"../input_files/{store}_2025-05-01.csv", f"{output_dir}/{store}_2025-05-01.csv")
    shutil.copyfile(f"../input_files/{store}_2025-05-08.csv", f"{output_dir}/{store}_2025-05-08.csv")

    # Copy original discounts files if they exist
